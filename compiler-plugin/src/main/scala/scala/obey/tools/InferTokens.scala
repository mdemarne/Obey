...
object inferTokens {
  ...
  private def infer(tree: Tree, proto: Option[Tree])(implicit dialect: Dialect): Tokens = {
    ...
    def tokensWithParens(tree: Tree): Tokens = {
      ...
      if (withParents) toks"(${deindent(tree.myTokens)})" else deindent(tree.myTokens)
    }
    ...
    def tkz(tree: Tree): Tokens = tree match {
      ...
      case t: Template =>
        ...
        if (isTemplateEmpty) toks""
          ...
          val pbody = {
            val isOneLiner = t.stats.map(stats => stats.length == 0 || (stats.length == 1 && !stats.head.myTokens.map(_.show[Code]).mkString.contains(EOL))).getOrElse(true)
      ...
      case t: Case =>
        ...
        val isOneLiner = {
          def isOneLiner(t: Case) = t.stats.length == 0 || (t.stats.length == 1 && !t.stats.head.myTokens.map(_.show[Code]).mkString.contains(EOL))
          t.parent match {
      ...
      case t: Source =>
        proto match {
          ...
          case Some(original: Source) =>
            val originalTokens = original.myTokens
            val oStatsTokens = original.stats.map(_.myTokens).repr
            val zipped = (oStatsTokens zip t.stats.map(_.myTokens.repr))
